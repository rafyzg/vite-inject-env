...
describe('vite-inject-env', () => {
  describe('Build', () => {
    ...
    it('should build file with placeholders from process.env', () => {
      ...
      expect(file).toContain('VITE_PROCESS_ENV: ViteInjectEnv_VITE_PROCESS_ENV')
    })
    it('should take in PUBLIC_URL env variable', () => {
      process.env['PUBLIC_URL'] = ''
      const file = buildFile()
      expect(file).toContain('PUBLIC_URL: ViteInjectEnv_PUBLIC_URL')
    })
    it('should build file with placeholders from dotenv file', () => {
      shell.exec('echo VITE_DOT_ENV = DOT_ENV > .env')
      const file = buildFile()
      expect(file).toContain('VITE_DOT_ENV: ViteInjectEnv_VITE_DOT_ENV')
    })
    it('should not overwrite variables that are not passed in', () => {
      const file = buildFile()
      expect(file).toContain('VITE_NO_INPUT: \n')
    })
    it('should not overwrite variables that do not start with VITE_', () => {
  ...
  describe('Inject', () => {
    ...
    it('should output to new folder without modifying the origin when using --o', () => {
      ...
      expect(readFile('tests/output/test4.txt')).toContain('VITE_INJECT_ENV4: ViteInjectEnv_VITE_INJECT_ENV4')
